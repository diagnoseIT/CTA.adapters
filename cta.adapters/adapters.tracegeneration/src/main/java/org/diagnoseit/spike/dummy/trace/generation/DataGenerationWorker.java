...
public class DataGenerationWorker implements Runnable {
	...
	private void interRecordSleep() {
		...
		long sleepTime = Math.max(AVG_INTER_DATA_TIME + (long) (pCent * (double) AVG_INTER_DATA_TIME), 1);
		try {
	...
	private void emitMonitoringRecord() {
		...
		rec.setCPUTime((long)(rand.nextDouble()*(double)rec.getDuration()));
		rec.setExecutionTime((long)(rand.nextDouble()*(double)rec.getDuration()));
		MonitoringDataProcessing.getInstance().addRecord(rec);
