...
int indent = 0;
		for (Callable callable : this) {
			...
			if (cImpl instanceof TraceInvocation) {
				...
				strBuilder.append(cImpl.getSignature()
						+ " - " + cImpl.getExecutionTime() + "ms   "   + cImpl.getContainingTrace().getLocation().getHost()+ ", "   + cImpl.getContainingTrace().getLocation().getRuntimeEnvironment());
			}
			strBuilder.append("\n");
